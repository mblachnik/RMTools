...
/**
 * Writes values of all examples into an ARFF file which can be used by the
 * machine learning library Weka. The ARFF format is described in the
 * {@link ArffExampleSource} operator which is able to read ARFF files to make
 * them usable with RapidMiner.
 *
 * @rapidminer.index arff
 * @author Ingo Mierswa
 */
public class KeelExampleSetWriter extends AbstractStreamWriter {
    ...
    public KeelExampleSetWriter(OperatorDescription description) {
        super(description);
    }
    ...
    public static void writeKeel(ExampleSet exampleSet, PrintWriter out, String linefeed) {
        if (linefeed == null) {
        ...
        while (a.hasNext()) {
            ...
            printAttributeData(exampleSet, attr, out);
            regularNames += attr.getName();            
        }
        // special attribute meta data
        Iterator<AttributeRole> ari = exampleSet.getAttributes().specialAttributes();
        ignoreFirst = false;
        while (ari.hasNext()) {
            AttributeRole ar = ari.next();
            String roleName = ar.getSpecialName();
            if (roleName.startsWith("label")) {   
                Attribute attr = ar.getAttribute();
                printAttributeData(exampleSet, attr, out);                                
                if (ignoreFirst) {
                    out.print(", ");
                }
                ignoreFirst = true;
                specialNames += attr.getName();                
            }
        } 
        //Write inputNames
        out.print("@inputs " + regularNames + linefeed);                      
        //Write outputNames
        out.print("@outputs " + specialNames + linefeed);               
        // data
        //out.print(linefeed);
        ...
        for (Example example : exampleSet) {
            ...
            ari = exampleSet.getAttributes().specialAttributes();
            while (ari.hasNext()) {
                AttributeRole ar = ari.next();
                if (!ar.getSpecialName().startsWith("label")){
                    continue;
                }
                Attribute current = ar.getAttribute();
                if (!first) {
                    out.print(",");
                }
    ...
    public static void writeKeel(ExampleSet exampleSet, PrintWriter out) {
        writeKeel(exampleSet, out, null);
    }

    private static void printAttributeData(ExampleSet exampleSet, Attribute attribute, PrintWriter out) {
